...
class WCMp_Frontend {
    ...
    public function display_vendor_msg_in_thank_you_page($order_id) {
        ...
        $is_csd_by_admin = '';
        $tracking_id = get_post_meta($order->get_id(), 'wcmp_vendor_order_tracking_id', true);
        $tracking_url = get_post_meta($order->get_id(), 'wcmp_vendor_order_tracking_url', true);
        $vendor_order_shipped = get_post_meta($order->get_id(), 'wcmp_vendor_order_shipped', true);
        ...
        if (!empty($vendor_array)) {
            ...
            if (apply_filters('can_customer_see_tracking_information', true) && $vendor_order_shipped) {
                $WCMp->template->get_template('vendor_order_tracking_info.php', array('tracking_id' => $tracking_id, 'tracking_url' => $tracking_url));
            }
